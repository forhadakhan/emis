/**
 * Parent: Activity.jsx;
 * Call From: ManageStudent.jsx(reference)
...
const ManageStudentProfile = ({ setActiveComponent, reference, breadcrumb }) => {
    ...
    updatedBreadcrumb = updatedBreadcrumb.concat(
        <button className='btn p-0 m-0' onClick={() => setActiveComponent('ManageStudentProfile')}>
            <i className="bi bi-person-gear"></i> Profile
        </button>
    );

    useEffect(() => {
        if (loggedUserRole === 'administrator' || hasPermission('view_student')) {
    ...
    const handleBlock = (activeState) => {
        if (loggedUserRole === 'administrator' || hasPermission('change_student')) {
    ...
    const handleDelete = () => {
        if (loggedUserRole === 'administrator' || hasPermission('delete_student')) {
    ...
    return (
        <div>
            ...
            {(loggedUserRole === 'administrator' || hasPermission('change_student')) &&
                <div className="container d-flex align-items-center justify-content-center">
                    <div className="btn-group m-5 " role="group" aria-label="Basic outlined example">
                        ...
                        {(loggedUserRole === 'administrator' || hasPermission('delete_student')) &&
